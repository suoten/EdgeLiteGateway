...
import asyncio, struct, sys, threading, time
...
from edgelite.drivers.fins import (
    FailoverInFlightTimeout, FinsConnState, FinsResponseError,
    FinsWriteError, OmronFinsDriver, PointHealthStats, WriteAuditEntry,
)
def _make_driver() -> OmronFinsDriver:
    d = OmronFinsDriver.__new__(OmronFinsDriver)
    for k,v in dict(_running=False,_health_stats={},_offline_since={},_device_configs={},
        _capabilities=DriverCapabilities(),_connection_statuses={},_reconnect_state={},
        _circuit_states={},_circuit_open_sinces={},_half_open_calls={},_failure_threshold=5,
        _recovery_timeout=30.0,_half_open_max_calls=3,_executor=None,_executor_max_workers=4,
        _executor_name_prefix="ft",_executor_futures=set(),_executor_futures_warn_threshold=64,
        _executor_shutting_down=False,_executor_shutdown_timeout=10.0,_background_tasks=set(),
        _delayed_reconnect_count=0,_permanent_offline=set(),_client=None,_config={},
        _socket_in_use=False,_reconnect_attempt=0,_reconnect_delay=1.0,_devices={},
        _watchdog_task=None,_watchdog_offline_count=0,_source_node=0,_dest_node=0,
        _network_no=0,_unit_no=0,_command_code="0101",_is_direct_mode=False,_plc_series="CJ",
        _MAX_LAST_VALUES=10000,_MAX_DICT_SIZE=10000,_conn_state=FinsConnState.DISCONNECTED,
        _primary_ip="",_primary_port=9600,_backup_ip="",_backup_port=0,_active_ip="",
        _active_port=9600,_using_backup=False,_primary_fail_count=0,_failover_probe_task=None,
        _point_configs={},_global_frozen_threshold=0,_global_rate_limit=0.0,_degraded=False,
        _degraded_interval_ms=0,_last_read_ts={},_frozen_counters={},_write_verify_enabled=False,
        _write_rate_limit_ms=500,_write_audit_enabled=True,_last_write_ts={},
        _quality_history={},_device_points={},_standby_client=None,_standby_task=None,
        _init_standby_task=None,_standby_ready=False,_standby_dest_node=0,_standby_network_no=0,
        _failover_ts=0.0,_delayed_reconnect_task=None,_bg_reconnect_task=None,
        _first_reconnect_time=0.0,_edge_rule_engine=None,_edge_trigger=None,_rule_store=None,
        _ts_store=None,_offline_sync=None,_network_online=True,_config_version_mgr=None,
        _audit=None,_ota_mgr=None,_current_user_role=None,_thread_pool=None,
        _thread_pool_failed=False,_thread_pool_created_at=0.0,
        _THREAD_POOL_MAX_AGE_SECONDS=3600.0,_is_udp=False,_udp_max_retries=3,_timeout=5.0,
        _max_response_size=65536,_data_callback=None).items(): setattr(d,k,v)
    d._reconnect_lock=asyncio.Lock(); d._circuit_lock=threading.Lock()
    d._executor_lock=asyncio.Lock(); d._shutdown_requested=threading.Event()
    d._stats_lock=threading.RLock(); d._conn_state_lock=threading.RLock()
    d._watchdog_exception_history=deque(maxlen=60); d._watchdog_history_lock=threading.Lock()
    d._lock=asyncio.Lock(); d._client_lock=threading.RLock(); d._async_client_lock=asyncio.Lock()
    d._in_flight_requests=0; d._in_flight_lock=threading.Lock()
    d._last_values=OrderedDict(); d._point_stats=OrderedDict()
    d._audit_log=deque(maxlen=1000); d._thread_pool_lock=asyncio.Lock()
    return d
def _mksock(recv_data=None):
    s=MagicMock(); s.fileno.return_value=1; s.settimeout=MagicMock(); s.send=MagicMock(); s.close=MagicMock()
    ...
    else: s.recv=MagicMock(return_value=b"")
    return s
def _mkclient(sock=None):
    c=MagicMock(); c.fins_socket=sock or MagicMock()
    c.read=MagicMock(return_value=42); c.write=MagicMock(return_value=None)
    c.close=MagicMock(); c.connect=MagicMock(); return c
def _ok_resp(data=b"\x00\x00"):
    return bytes(10)+struct.pack(">H",0)+bytes(2)+data
class TestParseAddress:
    @pytest.mark.parametrize("addr,exp", [("D100",("d",100,"w")),("CIO200",("c",200,"w")),("C300",("c",300,"w")),
        ("W400",("w",400,"w")),("H500",("h",500,"w")),("A600",("h",600,"w")),("EM700",("e",700,"w")),
        ("VM800",("v",800,"w")),("TK10",("tk",10,"w")),("CS20",("cs",20,"w")),("IR30",("ir",30,"w")),
        ("DR40",("dr",40,"w")),("CF50",("cf",50,"w"))])
    def test_areas(self, addr, exp):
        assert _make_driver()._parse_address(addr) == exp
    @pytest.mark.parametrize("addr,dt", [("D100,b","b"),("D100,w","w"),("D100,i","i"),("D100,r","r"),
        ("D100,ui","ui"),("D100,dw","dw"),("D100, b","b"),(" D100 , w ","w")])
    def test_data_types(self, addr, dt):
        assert _make_driver()._parse_address(addr)[2] == dt
    def test_invalid_address(self):
        with pytest.raises(ValueError, match="无效的FINS地址"): _make_driver()._parse_address("X100")
    def test_invalid_offset(self):
        with pytest.raises(ValueError, match="无效的FINS地址偏移量"): _make_driver()._parse_address("Dabc")
    def test_offset_out_of_range(self):
        with pytest.raises(ValueError, match="超出范围"): _make_driver()._parse_address(f"D{0xFFFFFF+1}")
class TestValidateWriteValue:
    @pytest.mark.parametrize("val,dt,ok", [(0,"b",True),(1,"b",True),(True,"b",True),(2,"b",False),
        (0,"w",True),(65535,"w",True),(65536,"w",False),("abc","w",False),(30000,"ui",True),
        (4294967295,"dw",True),(4294967296,"dw",False),(-32768,"i",True),(32767,"i",True),(32768,"i",False),
        (3.14,"float",True),(float("nan"),"float",False),(float("inf"),"float",False),
        (2.718,"r",True),(float("nan"),"r",False),(42,"unknown",True)])
    def test_validate(self, val, dt, ok):
        result, _ = _make_driver()._validate_write_value(val, dt)
        assert result is ok
class TestGetFinsAreaCode:
    @pytest.mark.parametrize("area,code", [("d",0x82),("e",0xA0),("v",0xA2),("c",0xB0),("w",0xB4),
        ("h",0xB8),("tk",0x18),("cs",0x30),("ir",0xDC),("dr",0xBC),("cf",0x28)])
...
class TestSetLastValue:
    def test_lru_eviction(self):
        d=_make_driver(); d._MAX_LAST_VALUES=3
        for i in range(4): d._set_last_value(f"D{i}",i)
        assert "D0" not in d._last_values and "D3" in d._last_values
    def test_move_to_end_on_update(self):
        d=_make_driver(); d._MAX_LAST_VALUES=3
        d._set_last_value("D1",1); d._set_last_value("D2",2); d._set_last_value("D3",3)
        d._set_last_value("D1",10); d._set_last_value("D4",4)
        assert "D1" in d._last_values and "D2" not in d._last_values
class TestPointHealthStats:
    def test_record_success(self):
        d=_make_driver(); d._record_point_success("D100",5.0); s=d._point_stats["D100"]
        assert s.success_count==1 and s.consecutive_fails==0 and s.avg_latency_ms==5.0
    def test_record_failure(self):
        d=_make_driver(); d._record_point_failure("D100"); s=d._point_stats["D100"]
        assert s.fail_count==1 and s.consecutive_fails==1
    def test_success_resets_fails(self):
        d=_make_driver(); d._record_point_failure("D100"); d._record_point_failure("D100")
        d._record_point_success("D100",1.0)
        assert d._point_stats["D100"].consecutive_fails == 0
    def test_lru_eviction(self):
        d=_make_driver(); d._MAX_DICT_SIZE=2
        d._record_point_success("D1",1); d._record_point_success("D2",2); d._record_point_success("D3",3)
        assert "D1" not in d._point_stats and "D3" in d._point_stats
    def test_check_degradation_no_stats(self):
        d=_make_driver(); d._check_degradation(); assert d._degraded is False
    def test_check_degradation_degraded(self):
        d=_make_driver()
        for _ in range(8): d._record_point_failure("D100")
        for _ in range(2): d._record_point_success("D100",1.0)
        d._check_degradation()
        assert d._degraded is True and d._degraded_interval_ms > 0
    def test_check_degradation_recovered(self):
        d=_make_driver(); d._degraded=True
        for _ in range(9): d._record_point_success("D100",1.0)
        d._record_point_failure("D100"); d._check_degradation()
        assert d._degraded is False and d._degraded_interval_ms == 0
    def test_get_point_health(self):
        d=_make_driver(); d._record_point_success("D100",5.0); h=d.get_point_health("D100")
    ...
    def test_get_point_stats(self):
        d=_make_driver(); d._record_point_success("D100",5.0); s=d.get_point_stats("dev1","D100")
        assert s is not None and s["current_quality"] == "good"
        assert d.get_point_stats("dev1","D999") is None
class TestCheckWriteRate:
    def test_disabled(self):
        d=_make_driver(); d._write_rate_limit_ms=0
        assert d._check_write_rate("D100") == (True,0.0)
    def test_allowed(self):
        d=_make_driver(); d._write_rate_limit_ms=500; d._last_write_ts["D100"]=0.0
        assert d._check_write_rate("D100")[0] is True
    def test_rate_limited(self):
        d=_make_driver(); d._write_rate_limit_ms=500; d._last_write_ts["D100"]=time.monotonic()
        ok,wait=d._check_write_rate("D100")
        assert ok is False and wait > 0
class TestAuditWrite:
    def test_disabled(self):
        d=_make_driver(); d._write_audit_enabled=False
        d._audit_write("dev1","D100","d",100,1,2,"ok")
        assert len(d._audit_log) == 0
    def test_enabled(self):
        d=_make_driver(); d._audit_write("dev1","D100","d",100,1,2,"ok")
        assert len(d._audit_log) == 1; e=d._audit_log[0]
        assert e.device_id=="dev1" and e.result=="ok"
    def test_with_fins_code(self):
        d=_make_driver(); d._audit_write("dev1","D100","d",100,1,2,"fins_error",fins_code=0x1101)
        assert d._audit_log[0].fins_code == "0x1101"
    def test_get_audit_log(self):
        d=_make_driver(); d._audit_write("dev1","D100","d",100,1,2,"ok")
        d._audit_write("dev1","D200","d",200,3,4,"error"); log=d.get_audit_log()
...
class TestStateManagement:
    def test_set_fins_state(self):
        d=_make_driver(); d._set_fins_state(FinsConnState.CONNECTED,"dev1","test")
...
class TestDeviceManagement:
    ...
    async def test_add_device_empty(self):
        d=_make_driver(); await d.add_device("dev1",{"host":"10.0.0.1"})
        assert "dev1" in d._devices
    def test_remove_device(self):
        d=_make_driver(); d._devices["dev1"]={"config":{},"points":{}}
        d._device_points["dev1"]=[{"name":"temp","address":"D100"}]
        d._point_configs["D100"]={"deadband":1}; d._health_stats["dev1"]=DriverHealthStats(device_id="dev1")
    ...
    async def test_health_check(self):
        d=_make_driver(); d._running=False
        assert await d.health_check("dev1") is False
        d._running=True; d._client=None
        assert await d.health_check("dev1") is False
        d._client=_mkclient()
        assert await d.health_check("dev1") is True
        c=MagicMock(); c.fins_socket=None; d._client=c
        assert await d.health_check("dev1") is False
class TestReadPoint:
    def test_normal_success(self):
        d=_make_driver(); d._is_direct_mode=False; c=_mkclient(); c.read.return_value=42; d._client=c
        assert d._read_point("D100") == 42
    def test_normal_no_client(self):
        d=_make_driver(); d._is_direct_mode=False; d._client=None
        with pytest.raises(ConnectionError): d._read_point("D100")
    def test_direct_success(self):
        d=_make_driver(); d._is_direct_mode=True; d._dest_node=1; d._unit_no=1; d._source_node=0
        d._client=_mkclient(); d._fins_tcp_request=MagicMock(return_value=42)
        assert d._read_point("D100") == 42
    def test_direct_no_client(self):
        d=_make_driver(); d._is_direct_mode=True; d._dest_node=1; d._unit_no=1; d._source_node=0; d._client=None
        with pytest.raises(ConnectionError, match="not connected"): d._read_point("D100")
    def test_direct_fallback(self):
        d=_make_driver(); d._is_direct_mode=True; d._dest_node=1; d._unit_no=1; d._source_node=0
        c=_mkclient(); c.read.return_value=99; d._client=c
        d._fins_tcp_request=MagicMock(side_effect=RuntimeError("err"))
        assert d._read_point("D100") == 99
class TestReadPointAsync:
    async def test_success(self):
        d=_make_driver(); d._config={"host":"10.0.0.1"}
...
class TestReadPointsBatch:
    ...
    async def test_fallback_split(self):
        d=_make_driver(); d._BATCH_SPLIT_MIN=2; call_count=0
        async def mock_batch(pts):
            nonlocal call_count; call_count+=1
            if call_count==1: raise RuntimeError("batch failed")
        ...
        with patch.object(d,"_read_points_batch",side_effect=mock_batch):
            with patch.object(d,"_read_point_async",new_callable=AsyncMock,
    ...
    async def test_fallback_max_depth(self):
        d=_make_driver(); d._BATCH_SPLIT_MIN=2
...
class TestReadPoints:
    async def test_not_running(self):
        d=_make_driver(); d._running=False; d._client=None; d._config={"host":"10.0.0.1"}
    ...
    async def test_success(self):
        d=_make_driver(); d._running=True; d._client=_mkclient()
        d._config={"host":"10.0.0.1","batch_size":10}; d._edge_rule_engine=None; d._ts_store=None
...
class TestWritePoint:
    async def test_not_running(self):
        d=_make_driver(); d._running=False; d._client=None
        with patch.object(d,"_handle_connection_failure",new_callable=AsyncMock,return_value=False):
    ...
    async def test_rate_limited(self):
        d=_make_driver(); d._running=True; d._client=_mkclient()
        d._write_rate_limit_ms=500; d._last_write_ts["D100"]=time.monotonic()
        assert await d.write_point("dev1","D100",1) is False
        assert d._audit_log[0].result == "rate_limited"
    async def test_write_success_normal(self):
        d=_make_driver(); d._running=True; d._client=_mkclient()
        d._is_direct_mode=False; d._write_verify_enabled=False
    ...
    async def test_write_success_direct(self):
        d=_make_driver(); d._running=True; d._client=_mkclient()
        d._is_direct_mode=True; d._dest_node=1; d._unit_no=1; d._source_node=0; d._write_verify_enabled=False
...
class TestFinsTcpRequestInner:
    @pytest.mark.parametrize("dt,data,exp", [("w",struct.pack(">H",12345),12345),
        ("b",bytes([0x01]),1),("dw",struct.pack(">I",100000),100000),
        ("float",struct.pack(">f",3.14),3.14),("i",struct.pack(">h",-100),-100)])
    def test_success(self, dt, data, exp):
        d=_make_driver(); d._timeout=5.0; d._max_response_size=65536; d._primary_ip="10.0.0.1"
        s=_mksock(_ok_resp(data)); d._client=MagicMock(); d._client.fins_socket=s
        result=d._fins_tcp_request_inner(bytes(12),dt)
        if dt=="float": assert abs(result-exp) < 0.001
        else: assert result == exp
    def test_no_socket(self):
        d=_make_driver(); c=MagicMock(); c.fins_socket=None; d._client=c
        with pytest.raises(RuntimeError, match="socket not available"):
            d._fins_tcp_request_inner(b"\x00"*12,"w")
    def test_invalid_header(self):
        d=_make_driver(); d._timeout=5.0; d._max_response_size=65536; d._primary_ip="10.0.0.1"
        s=MagicMock(); s.settimeout=MagicMock(); s.send=MagicMock()
        s.recv=MagicMock(side_effect=[b"XXXX"+struct.pack(">I",12),bytes(12)])
        d._client=MagicMock(); d._client.fins_socket=s
        with pytest.raises(RuntimeError): d._fins_tcp_request_inner(bytes(12),"w")
    def test_response_too_large(self):
        d=_make_driver(); d._timeout=5.0; d._max_response_size=100; d._primary_ip="10.0.0.1"
        s=MagicMock(); s.settimeout=MagicMock(); s.send=MagicMock()
        s.recv=MagicMock(side_effect=[b"FINS"+struct.pack(">I",200)])
        d._client=MagicMock(); d._client.fins_socket=s
        with pytest.raises(ValueError, match="too large"): d._fins_tcp_request_inner(bytes(12),"w")
    def test_fins_error_code(self):
        d=_make_driver(); d._timeout=5.0; d._max_response_size=65536; d._primary_ip="10.0.0.1"
        resp=bytes(10)+struct.pack(">H",0x0101)+bytes(2); s=_mksock(resp)
        d._client=MagicMock(); d._client.fins_socket=s
    ...
    def test_write_fins_error(self):
        d=_make_driver(); d._timeout=5.0; d._max_response_size=65536; d._primary_ip="10.0.0.1"
        resp=bytes(10)+struct.pack(">H",0x1101)+bytes(2); s=_mksock(resp)
        d._client=MagicMock(); d._client.fins_socket=s
...
class TestDoConnect:
    async def test_tcp(self):
        d=_make_driver(); d._config={"transport":"tcp","timeout":5}; mc=MagicMock(); mc.close=MagicMock()
    ...
    async def test_udp(self):
        d=_make_driver(); d._config={"transport":"udp","timeout":5}; mc=MagicMock(); mc.close=MagicMock()
...
class TestFinsNodeHandshakeSync:
    ...
    def test_no_socket(self):
        d=_make_driver(); c=MagicMock(); c.fins_socket=None
        assert d._fins_node_handshake_sync(c) is False
    def test_success(self):
        d=_make_driver(); d._network_no=0; d._dest_node=0; d._unit_no=0; d._source_node=0
        resp=bytes(6)+bytes([0x00,0x01])+bytes(2)+bytes([0x05,0x01])+bytes(4)
        s=MagicMock(); s.settimeout=MagicMock(); s.send=MagicMock(); s.close=MagicMock()
        header=b"FINS"+struct.pack(">I",len(resp))
        s.recv=MagicMock(side_effect=[header,resp]); c=MagicMock(); c.fins_socket=s
        assert d._fins_node_handshake_sync(c) is True
        assert d._dest_node == 0x01
    def test_invalid_header(self):
        d=_make_driver(); d._network_no=0; d._dest_node=0; d._unit_no=0; d._source_node=0
        s=MagicMock(); s.settimeout=MagicMock(); s.send=MagicMock(); s.close=MagicMock()
        s.recv=MagicMock(return_value=b"XXXX"+bytes(4)); c=MagicMock(); c.fins_socket=s
        assert d._fins_node_handshake_sync(c) is False
    def test_no_chunk(self):
        d=_make_driver(); d._network_no=0; d._dest_node=0; d._unit_no=0; d._source_node=0
        s=MagicMock(); s.settimeout=MagicMock(); s.send=MagicMock(); s.close=MagicMock()
        s.recv=MagicMock(return_value=b""); c=MagicMock(); c.fins_socket=s
        assert d._fins_node_handshake_sync(c) is False
...
class TestStart:
    @pytest.mark.parametrize("cfg,match", [({"port":0},"port"),({"network_no":200},"network_no"),
        ({"source_node":300},"source_node"),({"dest_node":300},"dest_node"),({"unit_no":300},"unit_no")])
    ...
    async def test_missing_host(self):
        with pytest.raises(ValueError, match="host"): await _make_driver().start({})
...
class TestStop:
    async def test_cleans_up(self):
        d=_make_driver(); d._running=True; d._client=_mkclient()
        d._point_stats["D100"]=PointHealthStats(); d._last_values["D100"]=42
...
class TestInitEnterprise:
    def test_init(self):
        d=_make_driver(); d.init_enterprise()
...
class TestWaitInFlight:
    async def test_no_pending(self):
        d=_make_driver(); d._in_flight_requests=0
        await d._wait_in_flight_requests(1.0)
    async def test_timeout(self):
        d=_make_driver(); d._in_flight_requests=5
...
class TestEnterpriseFeatures:
    ...
    @pytest.mark.parametrize("method,args,exp", [("ota_check_update",(MagicMock(),),False),
        ("ota_rollback",(),False),("ota_get_progress",(),{}),("ota_get_history",(),[]),
        ("get_audit_recent",(),[]),("get_audit_by_device",("dev1",),[]),
        ("get_audit_by_action",("write",),[]),("export_audit_csv",(),""),
        ("get_audit_stats",(),{}),("get_edge_rules",(),[])])
    def test_no_mgr(self, method, args, exp):
        assert getattr(_make_driver(),method)(*args) == exp
    @pytest.mark.parametrize("method,args,exp", [("save_config_version",("dev1",{}),0),
        ("get_config_current",("dev1",),None),("get_config_versions",("dev1",),[]),
        ("get_config_version_config",("dev1",1),None),("rollback_config",("dev1",1),None),
        ("force_sync_offline",(),0),("hot_reload_rules",(),0)])
...
class TestConfigSchema:
    def test_schema(self):
        ...
        for n in ["host","port","backup_host","transport","direct_mode","plc_series",
                   "deadband","scaling","clamp","frozen_threshold","rate_of_change_limit",
                   "write_verify","write_rate_limit_ms","write_audit"]:
            assert n in fields
