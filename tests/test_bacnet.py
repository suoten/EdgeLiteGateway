...
class TestBacnetWritePropertyApdu:
    ...
    async def test_write_property_apdu_service_choice_byte(self):
        ...
        task = asyncio.ensure_future(client.write_property(
            device_instance=100,
...
class TestSegmentReassembler:
    ...
    def test_two_segment_reassembly(self):
        ...
        # 第一段
        result = r.add_segment(invoke_id=1, sequence_number=0, more_follows=True,
                               service=12, data=b"\xAA\xBB")
        assert result is None  # 等待更多分段
        # 最后一段
        result = r.add_segment(invoke_id=1, sequence_number=1, more_follows=False,
                               service=None, data=b"\xCC\xDD")
