<template>
  <div class="twin-page">
    <div ref="containerRef" class="twin-canvas"></div>
    ...
    <div class="overlay-top">
      ...
      <div class="top-actions">
        ...
        <div :class="['action-btn', { active: showLabels }]" @click="showLabels = !showLabels" title="标签显示">🏷️</div>
    ...
    <div class="overlay-stats">
      ...
      <div class="stat-chip total">共 {{ deviceList.length }} 台设备</div>
    </div>
      ...
      <div v-if="selectedDevice" class="device-panel">
        ...
        <div class="panel-points">
          <div v-for="pt in (selectedDevice.points || []).slice(0, 8)" :key="pt.name" class="point-row">
            ...
            <span class="point-val">{{ formatPtVal(selectedDevice.device_id, pt) }}</span>
          </div>
      ...
      <div v-if="listExpanded" class="device-list-panel">
        ...
        <div class="device-scroll">
          <div
            ...
            :class="['dl-item', { selected: selectedDevice?.device_id === d.device_id }]"
            @click="flyToDevice(d)"
          >
            <div class="dl-dot" :style="{ background: d.status === 'online' ? '#18a058' : d.status === 'offline' ? '#d03050' : '#999' }"></div>
            <div class="dl-info">
...
let raycaster: any = null, mouse: any = null, refreshTimer: any = null
...
function buildDeviceMesh(device: any, THREE: any) {
  ...
  const bodyMat = new THREE.MeshStandardMaterial({
    ...
    emissive: isOnline ? color : 0x000000,
    emissiveIntensity: isOnline ? 0.2 : 0,
    transparent: !isOnline, opacity: isOnline ? 1.0 : 0.6,
  })
  ...
  if (device.protocol === 'modbus_tcp' || device.protocol === 'modbus_rtu') {
    ...
    const tip = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: isOnline ? 0x00ff88 : 0xff4444, emissive: isOnline ? 0x00ff88 : 0xff4444, emissiveIntensity: 0.8 })
    )
    tip.position.set(0.2, 1.2, 0)
    group.add(tip)
  } else if (device.protocol === 'video') {
    const lens = new THREE.Mesh(
      new THREE.CylinderGeometry(0.15, 0.2, 0.15, 16),
    ...
    const dish = new THREE.Mesh(
      new THREE.SphereGeometry(0.25, 16, 8, 0, Math.PI * 2, 0, Math.PI / 3),
      new THREE.MeshStandardMaterial({ color: 0xdddddd, metalness: 0.5, side: THREE.DoubleSide })
    )
    dish.position.y = 1.0
    ...
    const led = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: isOnline ? 0x00ff88 : 0xff4444, emissive: isOnline ? 0x00ff88 : 0xff4444, emissiveIntensity: 1.0 })
    )
  ...
  if (sceneType === 'factory') {
    ...
    for (let i = 0; i < count; i++) {
      ...
      positions.push({
        ...
        y: 0,
        z: (row - Math.ceil(count / cols) / 2 + 0.5) * spacing,
      })
    }
  ...
  if (selectedScene.value === 'factory') {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const building = new THREE.Mesh(
        new THREE.BoxGeometry(2 + Math.random() * 2, 3 + Math.random() * 4, 2 + Math.random() * 2),
        new THREE.MeshStandardMaterial({ color: 0x2a3a4a, metalness: 0.5, roughness: 0.5 })
      )
      building.position.set(Math.cos(angle) * r, building.geometry.parameters.height / 2, Math.sin(angle) * r)
...
function addDevicesToScene(THREE: any) {
  ...
  devices.forEach((device, i) => {
    ...
    group.userData = { deviceId: device.device_id, deviceName: device.name, floatOffset: i * 0.7 }
    scene.add(group)
...
.panel-id { font-size: 11px; color: #607d8b; margin-top: 4px; }
...
.point-name { color: #90a4ae; }
.point-val { color: #e0f0ff; font-weight: 600; font-variant-numeric: tabular-nums; }
.no-points { text-align: center; color: #607d8b; font-size: 12px; padding: 16px 0; }
...
.dl-meta { font-size: 11px; color: #607d8b; }
