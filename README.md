...
- **OPC-UA安全模式**：支持 None / Sign / SignAndEncrypt 三种安全模式 + 证书路径配置
- **S7自动重连**：指数退避重连（1s→60s）+ Rack/Slot 范围校验（0-7/0-31）+ 常见PLC型号提示
- **采集延迟监控**：实时统计每个设备的采集延迟/超时次数，`GET /collect-stats` 端点
- **数据质量评估**：跳变检测（quality=suspect）+ 范围校验（quality=out_of_range）
- **帧错误率告警**：设备错误率超 10% 自动降级告警
- **驱动看门狗**：3 周期未产出标记 stale，10 周期自动重启采集 Task
- **并发控制**：Semaphore 限制最大并发采集数（默认 50）
- **InfluxDB保留策略**：自动创建 30 天数据保留策略（可配置）
- **浮点精度保护**：`round(value, 6)` 避免浮点噪声

### 🎨 AI视觉增强（v1.0.2）

- **告警中心**：AI统计卡片 + 紫色高亮行 + AI渐变标签 + AI详情弹窗（异常分数仪表盘）+ 全部/AI/阈值筛选
- **AI模型管理**：引擎状态面板（紫色渐变）+ 预置模型卡片网格 + 推理测试增强（模拟数据+异常分数圆环）+ 性能进度条
- **仪表盘**：采集引擎面板（4统计卡片 + 活跃设备列表 + 数据瀑布动画）
- **设备列表**：采集状态列（脉冲点+频率）+ 今日数据列 + 在线/离线/异常筛选

### 🌐 i18n修复（v1.0.2）

- MCP工具描述 i18n 化
- Integration 列名 i18n 化
- 菜单"平台对接"改为"RPC反控"消除重复
