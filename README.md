...
在工业物联网领域，中小企业长期面临一个尴尬的选择困境：

| 方案 | 问题 |
|------|------|
| **ThingsBoard / 阿里IoT** | 部署复杂、资源占用高（4GB+内存）、按量计费贵、数据上云有顾虑 |
| **IoTGateway（.NET）** | 无规则引擎、无告警系统、无视频支持、前端技术陈旧（LayUI+jQuery） |
| **freeioe（Lua）** | 语言门槛极高，国内Lua开发者极少，二次开发困难 |
| **Node-RED** | 非企业级，无权限管理，流程复杂后难以维护 |

**EdgeLiteGateway 社区版** 专为解决这些痛点而生：
...
通过 PyGBSentry 适配器接入 GB28181 视频流，实现传感器数据与视频监控的统一管理。这是 **IoTGateway、freeioe、ThingsBoard 等所有竞品都不具备** 的核心能力。

### 3. 完整规则引擎 + 告警系统

| 功能 | EdgeLite | IoTGateway |
|------|:--------:|:----------:|
| 阈值告警 | ✅ | ❌ |
| 多条件 AND/OR | ✅ | ❌ |
| 持续时间窗口 | ✅ | ❌ |
...
工业协议全覆盖：Modbus TCP/RTU、OPC UA、OPC DA、西门子 S7、三菱 MC、欧姆龙 FINS、Allen Bradley、FANUC CNC、MTConnect、托利多称重、BACnet、DL/T 645、IEC 104、Sparkplug B、ABB/KUKA 机器人、ONVIF、串口设备、数据库接入、扫码枪。

### 5. 企业级安全体系

- JWT 认证 + RBAC 三角色 21 权限（admin/operator/viewer）
...
| MTConnect | 数控设备标准协议 |
| 托利多 (Toledo) | 称重仪表协议 |
| BACnet | 楼宇自控协议，支持 Who-Is 发现、属性读写 |
...
| `/api/v1/audit` | 审计日志（查询/导出/完整性校验/清理） |
...
| 版本 | 定位 | 核心特性 |
|------|------|----------|
| **v1.0 社区版** | 开源免费 | 17 种协议 + 规则引擎 + 告警 + 视频 + 内置 MQTT Server/Modbus Slave + 4 个北向平台 |
| **v1.2 企业版** | 商业授权 | + BACnet + OPC UA Server + 高可用 + 审计 + LDAP + 国密 + TDengine + Kafka |
| **v2.0 云边协同版** | 商业授权 | + K8s Operator + 边云同步 + 边缘 AI 推理 |

社区版对标 IoTGateway 开源版全功能，并增加规则引擎、告警系统、视频一体化等差异化能力。企业版提供 IoTGateway 开源版缺失的企业级特性。

---

## 竞品对比

| 功能 | EdgeLite 社区版 | IoTGateway | freeioe | ThingsBoard |
|------|:---------------:|:----------:|:-------:|:-----------:|
| 核心语言 | **Python** | C#/.NET | Lua | Java |
| 部署复杂度 | **⭐极低** | ⭐⭐低 | ⭐⭐低 | ⭐⭐⭐高 |
| 内存占用 | **<512MB** | ~512MB | 极低 | 4GB+ |
| 规则引擎 | **✅完整** | ❌仅表达式 | 应用框架 | ✅ |
| 告警系统 | **✅完整** | ❌ | ❌ | ✅ |
| 视频接入 | **✅GB28181** | ❌ | ❌ | ❌ |
| 时序存储 | **✅InfluxDB** | ❌ | ❌ | ✅ |
