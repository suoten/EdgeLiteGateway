...
class TumblingWindow:
    ...
    def __init__(self, size_seconds: float):
        ...
        self._buffer: deque[StreamEvent] = deque(maxlen=int(size_seconds * 100))  # 假设最高100点/秒
...
class StreamComputeEngine:
    ...
    def __init__(self):
        ...
        self._window_results: dict[str, float] = {}
    ...
    async def start(self, event_bus: EventBus | None = None) -> None:
        ...
        if event_bus is not None:
            ...
            self._subscriber_queue = event_bus.subscribe("stream_compute")
    ...
    def add_rule(self, rule_id: str, rule_config: dict) -> None:
        """添加流计算规则"""
    ...
    def remove_rule(self, rule_id: str) -> None:
        ...
        # 清理该规则关联的滑动窗口和 Processor
        self._rule_windows.pop(rule_id, None)
    ...
    async def _process_rule(self, rule_id: str, rule: dict, event: StreamEvent) -> list[StreamEvent]:
        ...
        if rule_type == "filter":
            ...
            # 获取或创建该规则对应的滑动窗口实例
            if rule_id not in self._rule_windows:
                self._rule_windows[rule_id] = SlidingWindow(
                    size_seconds=window_seconds,
                    slide_seconds=slide_seconds,
                    agg_func=agg_func,
                    min_count=min_count,
                )
            window = self._rule_windows[rule_id]
    ...
    def _eval_predicate(predicate: str, value: float) -> bool:
        ...
        try:
            # 安全评估，只支持基本比较
            safe_ops = {"<": "<", ">": ">", "==": "==", "!=": "!=", "<=": "<=", ">=": ">="}
            for op, symbol in safe_ops.items():
                if op in predicate:
    ...
    def get_window_result(self, device_id: str, point_name: str,
        ...
        return self._window_results.get(key)
