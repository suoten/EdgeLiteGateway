...
class RuleRepo:
    ...
    async def list_enabled_by_point(self, device_id: str, point_name: str) -> list[dict]:
        ...
        # 过滤包含指定测点的规�?        return [r for r in rules if any(c["point"] == point_name for c in r["conditions"])]
...
class AuditRepo:
    ...
    async def create(self, data: dict) -> None:
        await self.conn.execute(
            "INSERT INTO audit_logs (user_id, username, action, resource_type, resource_id, ip_address, user_agent, details, status, error_message) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                data["user_id"],
                data.get("username"),
