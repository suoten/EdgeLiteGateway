...
thorough testing. For production deployments, consider using the mature
`dnp3-python` (OPENDNP3) binding with DVP conformance certification.
...
- IIN (Internal Indications) 双字标志位完整解析
- 传输层分段重组 (_reassemble_response, _MAX_REASSEMBLE_FRAMES=64)
- CRC-16/DNP 校验 (polynomial 0x3D65)
"""
...
@dataclass
class DNP3IIN:
    """DNP3 Internal Indications (IIN) 双字标志位完整解析。

    参考 IEEE 1815-2012 § 11.3.2 Application Layer IIN Field。
    IIN1 (Byte 1) 与 IIN2 (Byte 2) 共 16 个标志位。
    """

    # IIN1 标志位
    all_stations: bool = False  # Bit 7: 广播地址匹配
    class_1_data: bool = False  # Bit 6: Class 1 数据可用
    class_2_data: bool = False  # Bit 5: Class 2 数据可用
    class_3_data: bool = False  # Bit 4: Class 3 数据可用
    time_sync_required: bool = False  # Bit 3: 需要时间同步
    local_control: bool = False  # Bit 2: 本地控制模式 (DOS)
    device_trouble: bool = False  # Bit 1: 设备故障 (DEL)
    event_buffer_overflow: bool = False  # Bit 0: 事件缓冲区溢出
    # IIN2 标志位
    config_corrupt: bool = False  # Bit 7: 配置损坏
    unknown_function: bool = False  # Bit 6: 不支持的功能码
    unknown_object: bool = False  # Bit 5: 不支持的对象
    parameter_error: bool = False  # Bit 4: 参数错误
    no_response: bool = False  # Bit 3: 对象未知
    state_inconsistent: bool = False  # Bit 2: 对象状态不一致
    header_mismatch: bool = False  # Bit 1: 头部不匹配
    object_syntax_error: bool = False  # Bit 0: 对象语法错误
    ...
    def has_critical_error(self) -> bool:
        ...
        return (
            self.device_trouble
            or self.event_buffer_overflow
            or self.config_corrupt
            or self.parameter_error
            or self.object_syntax_error
            or self.state_inconsistent
        )

    @property
    def has_data_available(self) -> bool:
        """是否有 Class 1/2/3 数据待读取。"""
        return self.class_1_data or self.class_2_data or self.class_3_data
...
class DNP3Client:
    ...
    def __init__(self, host: str, port: int = DEFAULT_DNP3_PORT, timeout: float = 10.0):
        ...
        # FIXED-P0: 原问题-IIN字节被直接丢弃(offset+=1三次)，未解析任何标志位；
        # 修复-完整解析 IIN1/IIN2 共16个标志位并持久化，便于上层判断设备故障/事件溢出/时间同步需求等
        self._last_iin: DNP3IIN | None = None

    @staticmethod
    def _parse_iin(iin1: int, iin2: int) -> DNP3IIN:
        """解析 DNP3 应用层 IIN (Internal Indications) 双字节标志位。

        参考 IEEE 1815-2012 § 11.3.2。IIN1 高位在前，IIN2 高位在前。
        """
        return DNP3IIN(
            # IIN1 (Byte 1)
            all_stations=bool(iin1 & 0x80),
            class_1_data=bool(iin1 & 0x40),
            class_2_data=bool(iin1 & 0x20),
            class_3_data=bool(iin1 & 0x10),
            time_sync_required=bool(iin1 & 0x08),
            local_control=bool(iin1 & 0x04),
            device_trouble=bool(iin1 & 0x02),
            event_buffer_overflow=bool(iin1 & 0x01),
            # IIN2 (Byte 2)
            config_corrupt=bool(iin2 & 0x80),
            unknown_function=bool(iin2 & 0x40),
            unknown_object=bool(iin2 & 0x20),
            parameter_error=bool(iin2 & 0x10),
            no_response=bool(iin2 & 0x08),
            state_inconsistent=bool(iin2 & 0x04),
            header_mismatch=bool(iin2 & 0x02),
            object_syntax_error=bool(iin2 & 0x01),
        )
    ...
    def _parse_response(self, data: bytes) -> list[DNP3Point]:
        ...
        # 应用层头: AC(1) + FC(1) + IIN1(1) + IIN2(1) = 4 bytes
        if offset + 4 > len(data):
            return points

        # FIXED-P0: 原问题-AC/FC/IIN1/IIN2 四个字节被直接读取并丢弃(data[offset]; offset+=1)，
        # 其中 IIN 字段是 DNP3 设备状态的关键诊断信息，丢失会导致无法感知设备故障/事件溢出等。
        # 修复-完整解析 AC/FC/IIN 并存储到 self._last_iin，关键错误记录日志告警。
        ...
        iin = self._parse_iin(iin1, iin2)
        self._last_iin = iin

        # 诊断日志：关键 IIN 标志告警
        if iin.has_critical_error:
            flags = [
                name
                for name in (
                    "device_trouble",
            ...
            logger.warning(
                "DNP3 device %s reports critical IIN flags: %s (IIN1=0x%02X IIN2=0x%02X, FC=0x%02X, AC=0x%02X)",
        ...
        if iin.time_sync_required:
            logger.info(
                "DNP3 device %s requires time synchronization (IIN1.3)",
                self._device_address,
            )
        ...
        while offset < len(data) - 1:
            try:
                ...
                offset += 1
                # FIXED-P0: 原问题-限定词(qualifier)字节被直接丢弃，未解析；
                # 修复-解析 qualifier 的 R (range field size) 与 Q (index mode)，
                # 校验与"16位索引范围"假设一致，不一致时跳过该对象头避免错误解析。
                qualifier = data[offset]
                offset += 1

                # qualifier 解析：高2位=范围字段大小(0:1B,1:2B,2:4B,3:保留),
                # 次高2位=模式(0:无范围,1:数量,2:索引范围,3:保留)
                range_field_size = (qualifier >> 6) & 0x03  # 0->1B, 1->2B, 2->4B
                range_mode = (qualifier >> 4) & 0x03  # 2 = 索引范围
                bytes_per_field = (1, 2, 4)[range_field_size] if range_field_size < 3 else 0

                if bytes_per_field != 2 or range_mode != 2:
                    logger.warning(
                        "DNP3 unsupported object header qualifier 0x%02X "
                        "(group=%d var=%d, range_field=%dB mode=%d); skipping object header",
                        qualifier,
                        group,
                        variation,
                        bytes_per_field,
                    ...
                    # 无法安全跳过（不知道范围字段总长），终止解析避免错位
                    break

                # 读取范围 (16-bit start/stop, 索引范围模式)
