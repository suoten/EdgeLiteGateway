...
class OmronFinsDriver(DriverPlugin):
    ...
    config_schema = {
        ...
        "required": ["host"],
        "properties": {"host": {"type": "string", "description": "PLC IP address", "format": "ipv4"}, "port": {"type": "integer", "description": "FINS TCP port", "minimum": 1, "maximum": 65535}},
        "fields": [
            ...
            {"name": "backup_host", "type": "string", "label": "Backup IP", "description": "Backup PLC IP for link redundancy. Auto-switch after 3 primary failures", "default": ""},
            {"name": "backup_port", "type": "integer", "label": "Backup Port", "description": "Backup PLC FINS port (default: same as port)", "default": 0},
            ...
            {"name": "source_node", "type": "integer", "label": "Source Node", "description": "Local FINS node number (default 0)", "default": 0, "min": 0, "max": 255},
            {"name": "dest_node", "type": "integer", "label": "Destination Node", "description": "Target FINS node number (default 0=auto, or 1 for CS/CJ2 direct mode)", "default": 0, "min": 0, "max": 255},
            {"name": "network_no", "type": "integer", "label": "Network No.", "description": "FINS network number: 0=local/direct (CS/CJ2), 1-127 for network", "default": 0, "min": 0, "max": 255},
            {"name": "unit_no", "type": "integer", "label": "Unit No.", "description": "FINS unit number: 0x00=CPU, 0x01=built-in Ethernet, 0xFF=broadcast", "default": 0, "min": 0, "max": 255},
            {"name": "command_code", "type": "string", "label": "Command Code", "description": "FINS command code for read operations", "default": "0101", "options": ["0101", "0102", "0103", "0104"]},
            {"name": "direct_mode", "type": "boolean", "label": "Direct Mode (CS/CJ2)", "description": "Enable FINS direct mode for CS/CJ2 series. Uses network_no=0, dest_node=1, unit_no=0x01 (built-in Ethernet port). No router required", "default": False},
            {"name": "plc_series", "type": "string", "label": "PLC Series", "description": "Omron PLC series: CJ (default), CS, CJ2, CP, NJ, NX", "default": "CJ", "options": ["CJ", "CS", "CJ2", "CP", "NJ", "NX", "auto"]},
            {"name": "deadband", "type": "object", "label": "Deadband", "description": "Deadband filter: {type: absolute|percent, threshold: number}. Skip reporting if change < threshold", "default": None},
            {"name": "scaling", "type": "object", "label": "Scaling", "description": "Linear scaling: {ratio: number, offset: number}. Applied as value*ratio+offset", "default": None},
            {"name": "clamp", "type": "object", "label": "Clamp", "description": "Range clamp: {min: number, max: number}. Values outside range are marked bad quality", "default": None},
            {"name": "frozen_threshold", "type": "integer", "label": "Frozen Threshold", "description": "Number of consecutive identical values before marking frozen (0=disabled)", "default": 0},
            {"name": "rate_of_change_limit", "type": "number", "label": "Rate of Change Limit", "description": "Max allowed change per second between reads (0=disabled)", "default": 0},
            {"name": "points_config", "type": "object", "label": "Points Config", "description": "Per-point config: {address: {deadband, scaling, clamp, frozen_threshold, rate_of_change_limit}}", "default": None},
            {"name": "write_verify", "type": "boolean", "label": "Write Verify", "description": "Enable write-back verification: write->delay 50ms->read back->compare", "default": False},
            {"name": "write_rate_limit_ms", "type": "integer", "label": "Write Rate Limit (ms)", "description": "Minimum interval between writes to the same register (default 500ms, 0=disabled)", "default": 500},
            {"name": "write_audit", "type": "boolean", "label": "Write Audit", "description": "Enable write operation audit logging", "default": True},
            {"name": "fins_max_response_size", "type": "integer", "label": "Max Response Size", "description": "Maximum FINS response size in bytes to prevent memory exhaustion (default 65536=64KB)", "default": 65536, "min": 1024, "max": 1048576},
        ],
    }

    experimental = False
    capabilities = DriverCapabilities(discover=True, read=True, write=True, subscribe=False, batch_read=True, batch_write=True)
    constraints = ({"type": "protocol_note", "message": "UDP mode may experience packet loss; source/dest node and network number must match remote configuration"},)  # FIXED(P2): 原问题-可变默认值list; 修复-改为tuple
            ...
            if self._thread_pool_failed:
                ...
                if old_pool is not None:
                    try:
                        ...
                        logger.warning("[fins] run_in_thread failed: %s", e)  # FIXED-P2: 原问题-异常被静默吞没，添加日志记录
        ...
        try:
            ...
            try:
                ...
                if c is not None and hasattr(c, 'fins_socket') and c.fins_socket is not None:
                    ...
                    logger.debug("[fins] code=SOCKET_CLOSED_AFTER_TIMEOUT msg=Closed fins_socket to interrupt blocking recv")
            except TimeoutError:
            ...
            try:
                ...
                try:
                    ...
                    loop.call_soon_threadsafe(
                        lambda: self._schedule_state_update(device_id, base_state, reason)
                    )
                except Exception as e:
                    logger.warning("[fins] set_fins_state failed: %s", e)  # FIXED-P2: 原问题-异常被静默吞没，添加日志记录

    def _schedule_state_update(self, device_id: str, base_state: str, reason: str) -> None:  # FIXED-P1: 线程安全的状态更新调度辅助方法
        try:
            loop = asyncio.get_event_loop()
            task = loop.create_task(self._set_connection_state(device_id, base_state, reason))
            self._background_tasks.add(task)
            ...
            logger.warning("[fins] schedule_state_update failed: %s", e)  # FIXED-P2: 原问题-异常被静默吞没，添加日志记录
    ...
    def _detect_frozen_value(self, address: str, value: Any) -> bool:
        ...
        if last is not None and value == last:
            if len(self._frozen_counters) >= self._MAX_DICT_SIZE and address not in self._frozen_counters:  # FIXED-P0: 字典无界增长，淘汰最旧条目
    ...
    def _check_degradation(self) -> None:
        ...
        if self._degraded and not was_degraded:
            ...
            self._log_error(FinsDriverErrors.FINS_DEGRADED_FREQ, "global", f"rate={rate:.2%} interval_ms={self._degraded_interval_ms}")
    ...
    def get_point_health(self, address: str) -> dict:
        stats = self._point_stats.get(address)
        if stats is None:
            return {"success_count": 0, "fail_count": 0, "avg_latency_ms": 0.0, "consecutive_fails": 0, "success_rate": 1.0}
        return {
    ...
    def _audit_write(self, device_id: str, point: str, area: str, offset: int,
                     old_value: Any, new_value: Any, result: str,
                     error_code: str = "", fins_code: int = 0,
                     verify_ok: bool | None = None) -> None:
        ...
        logger.info(
            ...
            entry.timestamp, entry.device_id, entry.point_id, entry.area_code,
            entry.address, entry.old_value, entry.new_value, entry.result,
            entry.error_code, entry.fins_code, entry.verify_ok,
        )
    ...
    def _merge_adjacent_writes(self, writes: list[tuple[str, Any]]) -> list[tuple[str, Any, bool]]:
        ...
        for point, value in writes:
            try:
                ...
                logger.warning("[fins] address parse failed for point %s: %s", point, e)  # FIXED-P1: 原问题-地址解析异常填充无效值无日志
    ...
    async def _fins_node_handshake(self, client=None, update_shared_state: bool = True) -> bool:
        ...
        return await self._run_in_thread(
            self._fins_node_handshake_sync, client, update_shared_state,
            timeout=15.0,
        )
    ...
    def _fins_node_handshake_sync(self, client=None, update_shared_state: bool = True) -> bool:
        ...
        try:
            ...
            fins_frame = bytes([
                0x80, 0x00, 0x02,
                self._network_no & 0xFF, self._dest_node & 0xFF, self._unit_no & 0xFF,
                0x00, self._source_node & 0xFF, 0x00,
                sid & 0xFF,
                0x05, 0x01,
            ])
            ...
            while len(header) < 8:
                if time.monotonic() > hs_deadline:  # FIXED-P0: 握手header读取总体超时
                    try: sock.close()
                    except Exception as e: logger.debug("[fins] handshake header timeout, sock.close failed: %s", e)  # FIXED-P2: 原问题-异常被静默吞没
                    return False
                chunk = sock.recv(8 - len(header))
                if not chunk:
                    try: sock.close()  # FIXED-P2: 握手失败时关闭socket，避免连接泄漏
                    except Exception as e: logger.debug("[fins] handshake no chunk, sock.close failed: %s", e)  # FIXED-P2
                    return False
                header += chunk
            if header[:4] != b"FINS":
                try: sock.close()  # FIXED-P2: 握手失败时关闭socket
                except Exception as e: logger.debug("[fins] handshake invalid header, sock.close failed: %s", e)  # FIXED-P2
                return False
            data_len = struct.unpack(">I", header[4:8])[0]
            # FINS-004: 检查响应大小上限
            if data_len > self._max_response_size:
                logger.error("[fins] code=RESPONSE_TOO_LARGE device=%s size=%d max=%d, closing connection", self._primary_ip, data_len, self._max_response_size)
                try:
                    sock.close()
                except Exception as e:
            ...
            while len(response) < data_len:
                if time.monotonic() > hs_deadline:  # FIXED-P0: 握手response读取总体超时
                    try: sock.close()
                    except Exception as e: logger.debug("[fins] handshake response timeout, sock.close failed: %s", e)  # FIXED-P2
                    return False
            ...
            if len(response) < 12:
                try: sock.close()
                except Exception as e: logger.debug("[fins] handshake response too short, sock.close failed: %s", e)  # FIXED-P2
                return False
            if len(response) < data_len:  # FIXED-P2: 响应不完整时拒绝处理，防止解析错误数据
                logger.warning("[fins] handshake response incomplete: got %d bytes, expected %d", len(response), data_len)
                try: sock.close()
                except Exception as e: logger.debug("[fins] handshake response incomplete, sock.close failed: %s", e)  # FIXED-P2
                return False
            mrc = response[10]
            src = response[11]
            ...
            with self._client_lock:
                if self._client:
                    try:
                        ...
                        logger.warning("[fins] connect_with_handshake failed: %s", e)  # FIXED-P2: 原问题-异常被静默吞没，添加日志记录
                    self._client = None
            return False
    ...
    async def start(self, config: dict) -> None:
        ...
        if self._is_direct_mode or self._plc_series in ("CS", "CJ", "CJ2", "CP"):
            ...
            if network_no == 0 and dest_node == 0:
                ...
                logger.info(
                    ...
                    self._primary_ip, self._plc_series,
                )
        ...
        if not ok and self._backup_ip:
            ...
            if self._primary_fail_count >= self._FAILOVER_THRESHOLD:
                ...
                self._log_error(FinsDriverErrors.FAILOVER_TRIGGERED, device_id, f"primary failed {self._primary_fail_count}x, switching to backup {self._backup_ip}")
                ok = await self._connect_with_handshake(device_id)
                if not ok:
    ...
    async def read_points(self, device_id: str, points: list[str]) -> dict[str, Any]:
        ...
        if quality < 60:
            ...
            if recovered:
                ...
                self._set_fins_state(FinsConnState.DEGRADED, device_id, "Connection quality below threshold after reconnect")
            ...
            if isinstance(pv, PointValue):
                if pv.quality == "good" and pv.value is not None:
                    ...
                    if not clamped_ok:
                        ...
                        result[p] = PointValue(value=clamped, quality="good", timestamp=pv.timestamp, latency_ms=pv.latency_ms)
    ...
    async def _read_points_batch_with_fallback(self, points: list[str], _depth: int = 0, _max_depth: int = 5, _deadline: float = 0.0) -> dict[str, Any]:
    ...
    def _read_point_direct_mode(self, area: str, offset: int, data_type: str) -> Any:
        try:
            ...
            fins_command = bytes([
                ...
                0x01, 0x01,
                area_code,
            ]) + offset_bytes + struct.pack(">H", word_count)
    ...
    async def _wait_in_flight_requests(self, timeout: float = 5.0) -> None:
        ...
        if count > 0:
            logger.error("[fins] code=IN_FLIGHT_TIMEOUT msg=%d in-flight requests still pending after %.1fs timeout", count, timeout)
    ...
    def _fins_tcp_request_inner(self, fins_command: bytes, data_type: str = "w") -> Any:
        ...
        try:
            ...
            if data_len > self._max_response_size:
                logger.error("[fins] device=%s code=RESPONSE_TOO_LARGE size=%d max=%d, closing connection", self._primary_ip, data_len, self._max_response_size)
                try:
                    sock.close()
                except Exception as e:
            ...
            if data_type == "b":
                if not data_bytes:
                    raise RuntimeError(f"FINS response data truncated: expected >=1 byte for bit, got {len(data_bytes)}")
                ...
                raise RuntimeError(f"FINS response data truncated: expected 2 bytes for signed int, got {len(data_bytes)}")
            else:
                if len(data_bytes) >= 2:
                    return struct.unpack(">H", data_bytes[:2])[0]
    ...
    async def _write_point_direct_mode(self, area: str, offset: int, value: Any, data_type: str) -> None:
        ...
        fins_command = bytes([
            ...
            0x01, 0x02,
            area_code,
        ]) + offset_bytes + struct.pack(">H", len(data_bytes)) + data_bytes
    ...
    async def write_point(self, device_id: str, point: str, value: Any) -> bool:
        ...
        if hasattr(self, "check_permission"):
            ...
            if not await self.check_permission(Permission.DEVICE_WRITE_POINT):
                self._log_error(FinsDriverErrors.RBAC_DENIED, device_id, f"role={self._current_user_role} lacks device:write_point")
                return False
        if not self._running or not self._client:
            recovered = await self._handle_connection_failure(device_id)
            if not recovered:
        ...
        if not valid:
            ...
            self._audit_write(device_id, point, area, offset, None, value, "rejected", error_code=FinsDriverErrors.WRITE_VALUE_OUT_OF_RANGE)
            return False
        ...
        if not allowed:
            ...
            self._audit_write(device_id, point, area, offset, None, value, "rate_limited", error_code=FinsDriverErrors.WRITE_RATE_LIMITED)
            return False
        ...
        try:
            ...
            if self._write_verify_enabled:
                ...
                if read_val is not None:
                    ...
                    if not verify_ok:
                        ...
                        self._audit_write(device_id, point, area, offset, old_value, value, "verify_failed", error_code=FinsDriverErrors.WRITE_VERIFY_FAILED, verify_ok=False)
                        return False
            ...
            self._audit_write(device_id, point, area, offset, old_value, value, "fins_error", error_code=error_code, fins_code=fins_code)
            return False
        except TimeoutError:  # FIXED-P1: 兼容Python<3.11
            self._record_write_failure(device_id)
            self._log_error(FinsDriverErrors.WRITE_TIMEOUT, point, f"{self._WRITE_TIMEOUT}s")
            self._audit_write(device_id, point, area, offset, old_value, value, "timeout", error_code=FinsDriverErrors.WRITE_TIMEOUT)
            return False
        except asyncio.CancelledError:
            raise
        except Exception as e:
            ...
            self._audit_write(device_id, point, area, offset, old_value, value, "error", error_code=FinsDriverErrors.WRITE_FAILED)
            return False
    ...
    async def batch_write_points(self, device_id, writes):
        ...
        results = await asyncio.gather(*tasks, return_exceptions=True)
        return {
            merged[i][0]: False if isinstance(r, Exception) else bool(r)
    ...
    async def _fast_failover(self, device_id: str) -> bool:
        ...
        if ok:
            ...
            self._log_error(FinsDriverErrors.FAILOVER_NO_BACKUP, self._backup_ip, f"failover took {elapsed:.2f}s but failed")
        return ok
            ...
            if not self._using_backup and self._standby_client is None and self._backup_ip:
                try:
                    ...
                    if _standby_init_fail_count >= 5:  # FIXED-P2: 连续5次初始化失败后暂停尝试
                        logger.warning("[fins] standby init failed %d times, pausing attempts", _standby_init_fail_count)
            ...
            if self._standby_client is not None:
                try:
                    ...
                    if not connected:
                        self._standby_ready = False
                        try:
                            if hasattr(self._standby_client, 'fins_socket') and self._standby_client.fins_socket is not None:
                                await asyncio.wait_for(asyncio.to_thread(self._standby_client.fins_socket.close), timeout=5.0)
                        except TimeoutError:
                            logger.warning("[fins] TCP writer close timeout (5s)")
                        except asyncio.CancelledError:
            ...
            try:
                ...
                for record in records:
                    if record.action == "firing":
                        self._log_error(FinsDriverErrors.EDGE_RULE_FIRED, device_id, f"rule={record.rule_id} point={record.point_name} value={record.trigger_value:.2f} threshold={record.threshold:.2f} latency={record.latency_ms:.1f}ms")
                    elif record.action == "recovered":
                        self._log_error(FinsDriverErrors.EDGE_RULE_RECOVERED, device_id, f"rule={record.rule_id} point={record.point_name}")
            except asyncio.CancelledError:
                raise
            except Exception as e:
    ...
    async def hot_reload_rules(self) -> int:
        ...
        for rule in new_rules:
            self._edge_rule_engine.add_rule(rule)
        self._log_error(FinsDriverErrors.RULE_HOT_RELOADED, "global", f"reloaded {len(new_rules)} rules (was {len(old_rules)})")
    ...
    async def query_ts(self, device_id: str, point_name: str, start_time, end_time=None, quality: str | None = None, aggregate: str | None = None, window_seconds: int | None = None, limit: int = 10000) -> list[dict]:
    ...
    async def rollback_config(self, device_id: str, target_version: int, operator: str = "system") -> dict | None:
        ...
        if not self.check_rbac(operator, "config_version:edit", device_id):
            self._log_error(FinsDriverErrors.CONFIG_CHANGE_DENIED, device_id, f"rollback denied for operator={operator}")
            return None
        ...
        self._log_error(FinsDriverErrors.CONFIG_VERSION_ROLLBACK, device_id, f"to_version={target_version} operator={operator}")
        return config
    ...
    async def diff_config_versions(self, device_id: str, version_a: int, version_b: int) -> dict:  # FIXED-P0: 改为async并await，原同步调用async方法返回协程对象而非dict
        if not self._config_version_mgr:
            return {"changes": []}
    ...
    async def _try_reconnect(self, device_id: str) -> None:
        ...
        async with self._reconnect_lock:
            ...
            self._log_error(FinsDriverErrors.CONN_LOST, device_id, f"reconnect in {delay:.2f}s (attempt {self._reconnect_attempt})")
    ...
    async def _delayed_reconnect(self, delay: float, device_id: str) -> None:
        ...
        # 修复：检查device_id是否仍在self._devices中，若已移除则直接返回
        if device_id not in self._devices:
            logger.info("[fins] device=%s code=DELAYED_RECONNECT_CANCELLED msg=Device removed, skipping delayed reconnect", device_id)
            return
        ...
        if self._first_reconnect_time == 0.0:
            ...
            self._log_error(FinsDriverErrors.CONN_LOST, device_id, "delayed reconnect exceeded 24h window, marking device permanently offline")
        ...
        if self._delayed_reconnect_count > self._MAX_DELAYED_RECONNECTS:
            self._log_error(FinsDriverErrors.CONN_LOST, device_id, f"delayed reconnect reached max {self._MAX_DELAYED_RECONNECTS} attempts, marking permanently offline")
            self._permanent_offline.add(device_id)  # FIXED-P2: 达上限后加入永久离线集合，而非重置计数器继续循环
            self._delayed_reconnect_count = 0
            return
        self._log_error(FinsDriverErrors.CONN_LOST, device_id, f"delayed reconnect in {delay:.0f}s (attempt {self._delayed_reconnect_count}/{self._MAX_DELAYED_RECONNECTS})")
        await asyncio.sleep(delay)
        self._reconnect_attempt = 0
        self._reconnect_delay = self._RECONNECT_BASE_DELAY
        ...
        while self._running:
            ...
            try:
                ...
                if ok:
                    ...
                    self._log_error(FinsDriverErrors.FAILOVER_NO_BACKUP, self._primary_ip, "primary reconnect failed after probe, staying on backup")
            except asyncio.CancelledError:
                raise
            except Exception:
    ...
    async def _watchdog_loop(self) -> None:
        ...
        while self._running:
            ...
            try:
                ...
                connected = (
                    wd_client is not None
                    and hasattr(wd_client, "fins_socket")
                    and wd_client.fins_socket is not None
                )
                if connected:
                    if self._watchdog_offline_count > 0:
                        logger.info("[fins] watchdog: device recovered, resetting offline count from %d", self._watchdog_offline_count)
                    ...
                    if device_id not in self._offline_since:
                        ...
                        logger.warning("[fins] watchdog: device offline (count=%d, interval=%.0fs), %s",
                                       self._watchdog_offline_count, interval, device_id)
                    elif (now - self._offline_since[device_id]).total_seconds() > 30:  # FIXED-P0: _offline_since统一使用datetime类型
                        logger.warning("[fins] watchdog: connection offline >30s (count=%d), triggering reconnect for %s",
                                       self._watchdog_offline_count, device_id)
                        await self._try_reconnect(device_id)
            except asyncio.CancelledError:
                raise
            except Exception as e:
    ...
    async def health_check(self, device_id: str) -> bool:
        ...
        try:
            connected = (
                hc_client is not None
                and hasattr(hc_client, "fins_socket")
                and hc_client.fins_socket is not None
            )
            return connected
        except asyncio.CancelledError:
            raise
